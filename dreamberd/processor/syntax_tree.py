...
class CodeStatementKeywordable(
    metaclass=ABCMeta
):  # this is completely unnecessary lmao
    keyword: Token
...
class VariableAssignment(CodeStatement, CodeStatementDebuggable):
    ...
    debug: int
    indexes: Union[
        list[list[Token]], list[ExpressionTreeNode]
    ]  # list[Token] here is an expression not evaled yet
    confidence: int
